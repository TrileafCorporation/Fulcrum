...
function getFieldDocsPhotosPath(branch, projectNumber) {
  ...
  if (!fs.existsSync(basePath)) {
    console.warn(`Branch directory does not exist: ${basePath}. Returning unexciting path.`);
  ...
  if (!projectDirEntry) {
    console.warn(`No directory starts with "${projectNumber}" under ${basePath}. Returning unexciting path.`);
    return path.join(filePath, 'unexciting');
  }
  ...
  if (!fs.existsSync(fieldDocsPath)) {
    try {
      ...
      throw new Error(`Failed to create "Field Docs" folder at: ${fieldDocsPath}. Error: ${err.message}`);
    }
  } else {
  ...
  if (!fs.existsSync(photosPath)) {
    try {
      ...
      throw new Error(`Failed to create "Photos" folder at: ${photosPath}. Error: ${err.message}`);
    }
  } else {
...
function getFieldDocsPhotosPath_no_matter_what(branch, projectNumber) {
  console.log(`Using branch: ${branch}`);
  ...
  if (!fs.existsSync(basePath)) {
    console.warn(`Branch directory does not exist: ${basePath}. Returning unexciting path.`);
    return path.join('\\\\trileaf.local\\Project_Folders', 'unexciting');
  }
  ...
  // Check for "Photos" folder inside the "Field Docs" folder. Create it if it doesn't exist.
  const photosPath = path.join(fieldDocsPath, 'Photos_recovered_final');
  if (!fs.existsSync(photosPath)) {
    try {
      fs.mkdirSync(photosPath, { recursive: true });
      console.log(`"Photos" folder created at: ${photosPath}`);
  ...
  try {
    ...
    for (const record of records.objects) {
      ...
      try {
        ...
        const photos_check = await check_for_new_photos(record.id, look_up_array);
        ...
        const photos = await getFilesInFolder('./app/util/photos', { onlyFiles: true });
        ...
        for (const relativePhotoPath of photos) {
          ...
          await copyImageToFolder(absolutePhotoPath, newPhotoFolder, photo_array, secondary_folder_name, projectNum, fieldVisitNotes);
        }
        ...
        console.error(`Error processing record ID ${record.id}: ${recordError}`);
      }
    }
...
app.post('/savephotos', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Failed to save photos.', error: error.message });
  }
});


// Route to trigger the photo-saving process manually
app.post('/savephotosnomaterwhat', async (req, res) => {
  try {
    await savePhotosProcess_no_matter_what();
    res.status(200).json({ message: 'Photos saved successfully.' });
  } catch (error) {
    res.status(500).json({ message: 'Failed to save photos.', error: error.message });
  }
